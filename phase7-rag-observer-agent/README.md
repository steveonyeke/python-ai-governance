...
Phase 7 complete. I built a three-layer agentic
governance pipeline and found two critical
architectural flaws in my own system.

Here is what the pipeline does:

Layer 1: Semantic Intent Check
Every query passes through a pre-execution
classifier that strips wrapper framing and
evaluates the underlying intent. Translation
attacks, hypothetical framing, roleplay wrapping,
direct injection: all intercepted before the
pipeline runs.

Layer 2: RAG Pipeline
Legitimate queries retrieve relevant regulatory
documents from a knowledge base covering EU AI Act
Articles 5, 10, 14, 99 and NIST AI RMF and
NIST AI 600-1. The LLM answers only from
retrieved sources. Every claim is traceable to
a specific regulatory document.

Layer 3: Observer Agent
Every RAG response is independently audited before
reaching the user. Grounding score. Source relevance
score. Factual integrity score. Three-tier verdict:
APPROVED, FLAGGED, or REJECTED.

Here are the two architectural flaws I found:

Flaw 1: Same-family Observer bias.
Both the RAG Agent and Observer Agent use the same
model. The Observer gave perfect 5/5 scores on
every query. This replicates the Phase 3 finding
where Gemini judged Gemini text outputs as perfect.
The Observer cannot detect blind spots it shares
with the RAG Agent.
Production fix: cross-model auditing. Different
model family for the Observer.

Flaw 2: Semantic classifier non-determinism.
The same academic research framing query was
blocked in one run and allowed in another.
LLM classifiers are probabilistic. They cannot
guarantee consistent verdicts for identical inputs.
Production fix: temperature 0, confidence scoring,
human review for borderline cases.

Overall: 88-100% semantic intent accuracy.
Translation wrapper attacks blocked 2/2.
Observer Agent 3/3 APPROVED.
RAG grounding 4/4 confirmed.
...
#AIGovernance #AgenticAI #RAG #LLMEvaluation
#ObserverAgent #ResponsibleAI #Python #BuildInPublic
